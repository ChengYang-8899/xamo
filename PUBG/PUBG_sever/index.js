...
server.get("/strategy", (req, res) => {
  ...
  var offsset = (ppo - 1) * ps;
  ps = parseInt(ps);
  var sql = "SELECT id,title,text,pic FROM pu_strategy LIMIT ?,?";
  pool.query(sql, [offsset, ps], (err, result) => {
    if (err) throw err;
    res.send({ code: 1, msg: "查询成功", data: result });
  });
});
