<template>
  <div>
    ...
    <div @click="ulist" class="a">
      <div class="list">好友</div>
      <div>▽</div>
    </div>
    <div v-show="b==-1">
      <span>{{msg}}</span>
      <div v-for="(p,i) of kong" :key="i" class="a">
        <img class="avatar" :src="'http://127.0.0.1:5050/'+p.avatar" />
        <span>{{p.uname}}</span>
      </div>
    </div>
    <div @click="sq" class="a">
      <div class="list">申请</div>
      <div>▽</div>
    </div>
    <div v-show="c==-1">
      <span>{{smsg}}</span>
      <div v-for="(p,i) of txmc" :key="i" class="a">
        ...
        <div>
          <span class="friend-name">拒绝</span>
        </div>
      </div>
    </div>
    <div class="room">聊天室</div>
    <div class="a" @click="haidao">
      <div class="list">海岛</div>
      <div>▽</div>
    </div>
    <div class="a">
      <div class="list">沙漠</div>
      <div>▽</div>
    </div>
    <div class="a">
      <div class="list">雨林</div>
      <div>▽</div>
    </div>
    <div>
      <span>{{bbb}}</span>
      <div class="a" :style="{ 'height':height,'opacity':opacity }">
        <img class="avatar" :src="'http://127.0.0.1:5050/'+sqhy.avatar" />
        <span>{{sqhy.uname}}</span>
        <div>
          <span @click="fssq" class="friend-name">申请</span>
        </div>
        <div>
          <span @click="qxsq" class="friend-name">取消</span>
        </div>
      </div>
      <div class="null"></div>
    </div>
...
export default {
  ...
  methods: {
    ...
    fssq() {
      this.ulist();
      for (var p of this.hy) {
        if (p == this.hyid.toString()) {
          this.$toast("此人以是你的好友");
          this.height = "0px";
          this.opacity = "0";
          return;
    ...
    onSearch() {
      ...
      if (account !== "") {
        ...
        this.axios.get(url, { params: obj }).then(res => {
          if (res.data.code == 1) {
            this.hyid = res.data.data[0].id;
            this.sqhy = res.data.data[0];
            this.height = "40px";
            this.opacity = "1";
          } else {
            this.height = "0";
